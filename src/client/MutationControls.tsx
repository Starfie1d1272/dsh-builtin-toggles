...
import { availableActions, type Capability, type InspectionSnapshot, type MutationAction } from './inspector-model.ts'
...
export function MutationControls({ capability, snapshot, busy, onMutate, t }: { capability: Capability; snapshot: Pick<InspectionSnapshot, 'access'>; busy: boolean; onMutate: (id: string, action: MutationAction) => void; t: BuiltinTogglesTabProps['t'] }): JSX.Element {
  ...
  if (actions.length === 0) return <p style={muted}>{snapshot.access.mutation === 'loopback-required' ? t('remoteReadOnly') : `${t('controlsUnavailable')}: ${capability.mutationEligibility.reasons.map((reason) => t(`eligibility${capitalize(reason)}` as never)).join(', ') || t('unknown')}`}</p>
