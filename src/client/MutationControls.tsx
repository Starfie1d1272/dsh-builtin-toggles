...
import { availableActions, wouldBeEligibleLocally, type Capability, type InspectionSnapshot, type MutationAction } from './inspector-model.ts'
...
export function MutationControls({ capability, snapshot, busy, onMutate, t }: { capability: Capability; snapshot: Pick<InspectionSnapshot, 'access'>; busy: boolean; onMutate: (id: string, action: MutationAction) => void; t: BuiltinTogglesTabProps['t'] }): JSX.Element {
  const actions = availableActions(capability, snapshot)
  if (actions.length === 0) {
    // Only rows that would be eligible under loopback access get a per-item
    // remote note. Everything else keeps its real policy/eligibility meaning;
    // a page-level banner explains the global remote read-only mode.
    if (snapshot.access.mutation === 'loopback-required' && wouldBeEligibleLocally(capability)) {
      return <p style={muted}>{t('remoteReadOnlyEligible')}</p>
    }
    if (capability.policy.status === 'locked') {
      // Locked rows keep a concise default reason; the full reasons and
      // limitations remain visible in the machine evidence.
      return <p style={muted}>{t('controlsUnavailable')}</p>
    }
    return <p style={muted}>{`${t('controlsUnavailable')}: ${capability.mutationEligibility.reasons.map((reason) => t(`eligibility${capitalize(reason)}` as never)).join(', ') || t('unknown')}`}</p>
  }
