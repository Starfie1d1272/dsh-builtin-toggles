...
export const zhCatalog: Readonly<Record<string, BuiltinCatalogEntry>> = {
  ...
  'tool-bash': presetManaged(
    'Bash 工具',
    '让 Agent 在 macOS/Linux 等环境执行 Shell 命令。',
    '工具与执行',
  ),
  'tool-pwsh': presetManaged(
    'PowerShell 工具',
    '让 Agent 在 Windows 环境执行 PowerShell 命令。',
    '工具与执行',
  ),
  ...
  'skill-filesystem': presetManaged(
    '本地 Skill 发现',
    '从文件系统发现并注册当前 Agent 可用的 Skill。',
    '模型与 Agent',
  ),
  'tool-skill': presetManaged(
    'Skill 工具',
    '让 Agent 查看和加载 Skill，并处理用户显式调用的 Skill。',
    '模型与 Agent',
  ),
  'tool-goal': presetManaged(
    'Goal 工具',
    '让 Agent 读取和更新持久化的会话 Goal。',
    '模型与 Agent',
  ),
  'plan-mode': presetManaged(
    'Plan 模式',
    '为 Agent 提供规划模式、退出工具和对应的规划规则。',
    '模型与 Agent',
  ),
  ...
  'tool-subagent-list-agents': presetManaged(
    '子代理列表工具',
    '让 Agent 查看可用和已创建的子代理。',
    '模型与 Agent',
  ),
  'tool-subagent': presetManaged(
    '创建子代理',
    '让 Agent 创建一个可继续交互的 Spawn 子代理。',
    '模型与 Agent',
  ),
  'tool-subagent-fork': presetManaged(
    'Fork 子代理',
    '让 Agent 从当前历史 fork 一个一次性子代理。',
    '模型与 Agent',
  ),
  'workflow-worker-thread': presetManaged(
    'Workflow 执行后端',
    '在线程工作器中执行持久 Workflow。',
    '工具与执行',
  ),
  'tool-workflow': presetManaged(
    'Workflow 工具',
    '让 Agent 启动和管理 Workflow。',
    '工具与执行',
  ),
  'tool-ralph': presetManaged(
    'Ralph 迭代工具',
    '按固定流程反复启动新的 Agent 轮次，用于多轮迭代执行。',
    '工具与执行',
  ),
  'agent-instructions': presetManaged(
    '工作区指令',
    '为 Agent 自动加载 AGENTS.md、CLAUDE.md 等工作区指令。',
    '模型与 Agent',
  ),
  ...
  'tool-web': presetManaged(
    'Web 搜索工具',
    '让 Agent 使用 web_search；官方默认组合只开放搜索，不开放任意 URL fetch。',
    '工具与执行',
  ),
