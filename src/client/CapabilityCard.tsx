...
import type { Capability, InspectionSnapshot, MutationAction } from './inspector-model.ts'
...
export function CapabilityCard({ capability, snapshot, busy, initiallyExpanded, domId, onMutate, t }: { capability: Capability; snapshot: InspectionSnapshot; busy: boolean; initiallyExpanded: boolean; domId: string; onMutate: (id: string, action: MutationAction) => void; t: BuiltinTogglesTabProps['t'] }): JSX.Element {
  ...
  return <li id={domId} style={card} aria-busy={busy || undefined}>
    <h3 style={title}>{t('capabilityName', { name: displayName })}</h3>
