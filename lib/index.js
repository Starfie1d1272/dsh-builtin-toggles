...
function inspectProfileOverride(content, id) {
	...
	try {
		...
		return {
			state: "unavailable",
			reason: message.includes("duplicate top-level") ? "duplicate_top_level_row" : message.includes("duplicate disabled") ? "duplicate_disabled_field" : "non_literal_disabled"
		};
	}
}
/**
...
function preflightProfileMutation(file, id) {
	...
	try {
		...
		return {
			status: "unwritable",
			reason: (error && typeof error === "object" && "code" in error ? error.code : void 0) === "ENOENT" ? "profile_patch_missing" : "profile_patch_unreadable"
		};
	}
	try {
...
function renderDisabledPatch(content, id, disabled) {
	...
	if (target !== null) {
		...
		} else lines.splice(target.end, 0, `${target.indent}disabled: ${value}`);
		return {
...
function buildInspectionResponse(entries, runtimeIdentity = null, profileOverrides = /* @__PURE__ */ new Map(), profilePersistence = /* @__PURE__ */ new Map()) {
	...
	const capabilities = entries.map((entry) => {
		...
		const writable = profilePersistence.get(entry.id) ?? { status: "writable" };
		return {
			id: entry.id,
			packageName: entry.name,
			...
			configuration: {
				profileOverride: profileOverrides.get(entry.id) ?? { state: entry.ownDisabled === true ? "explicitly-disabled" : entry.ownDisabled === false ? "explicitly-enabled" : "inherited" },
			...
			verification: findingCodesById.get(entry.id) ?? (reviewed === void 0 || !identityVerified ? "unverified" : "verified"),
...
function profileOverrideSnapshot(entries) {
	...
	try {
		...
		for (const entry of entries) states.set(entry.id, {
			state: "unavailable",
...
function profilePersistenceSnapshot(entries) {
	const states = /* @__PURE__ */ new Map();
	const file = profilePatchPath("web");
		...
		return ctx.webServer.register({
			...
			handler: async (req, res) => {
				...
				if (method === "GET" && pathname === INSPECTION_API_PATH) {
					...
					sendJson(res, 200, buildInspectionResponse(entries, null, profileOverrideSnapshot(entries), profilePersistenceSnapshot(entries)));
					return;
				}
				...
				if (method === "POST" && match !== null) {
					...
					const result = await serializeMutation(() => runToggle({
						...
						listEntries: () => [...ctx.loader.entries()].filter((entry) => !entry.options.group).map(entryHandle),
