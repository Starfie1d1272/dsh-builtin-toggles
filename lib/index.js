...
*/
function evaluateMutationEligibility(id, runtimeEntries, baseline = REVIEWED_DSH_WEB_BASELINE, compatibility = evaluateCompatibility(runtimeEntries, baseline)) {
	const reasons = [];
...
function inspectProfileOverride(content, id) {
	...
	try {
		...
		return {
			state: "unavailable",
			reason: (error instanceof Error ? error.message : "").includes("duplicate top-level") ? "duplicate_top_level_row" : "non_literal_disabled"
		};
	}
}
/**
...
async function runToggle(deps, id, rawBody) {
	...
	const eligibility = evaluateMutationEligibility(id, entries.map((candidate) => ({
...
function buildInspectionResponse(entries, runtimeIdentity = null, profileOverrides = /* @__PURE__ */ new Map()) {
	...
	const capabilities = entries.map((entry) => {
		...
		return {
			...
			mutationEligibility: evaluateMutationEligibility(entry.id, runtimeEvidence, REVIEWED_DSH_WEB_BASELINE, compatibility),
			baseline: {
