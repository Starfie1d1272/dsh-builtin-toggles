...
*/
const REVIEWED_RC6_RUNTIME_AUGMENTATION_PAIRS = /* @__PURE__ */ new Set([
	"0672880e\0@deepseek-ai/dsh-host-directory-picker-browse",
	"acd17651\0@deepseek-ai/dsh-client-ui-directory-picker-browse",
	"7038d3b5\0@deepseek-ai/cordis-plugin-hmr",
	"823ffe04\0@deepseek-ai/dsh-host-directory-picker-native",
	"0672880e\0@deepseek-ai/dsh-client-ui-directory-picker-native",
	"acd17651\0@deepseek-ai/cordis-plugin-hmr"
]);
function isReviewedBootstrapAugmentation(entry) {
...
function buildInspectionResponse(entries, runtimeIdentity, profile) {
	...
	const capabilities = entries.map((entry) => {
		...
		return {
			...
			verification: findingCodesById.get(entry.id) ?? (reviewed === void 0 ? "unverified" : !identityVerified ? "unverified" : "verified"),
		...
		return ctx.webServer.register({
			...
			handler: async (req, res) => {
				...
				if (method === "GET" && pathname === INSPECTION_API_PATH) {
					...
					sendJson(res, 200, buildInspectionResponse(entries, null, inspectProfileSnapshot(profilePatchPath("web"), entries.map((entry) => entry.id))));
					return;
				}
