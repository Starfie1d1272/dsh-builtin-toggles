...
function buildInspectionResponse(entries, runtimeIdentity = null) {
	...
	})), REVIEWED_DSH_WEB_BASELINE, runtimeIdentity);
		...
		return ctx.webServer.register({
			...
			handler: async (req, res) => {
				...
				if (method === "GET" && pathname === INSPECTION_API_PATH) {
					sendJson(res, 200, buildInspectionResponse([...ctx.loader.entries()].filter((entry) => !entry.options.group).map(inspectionEntry), null));
					return;
				}
