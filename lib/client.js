	...
	factory: (require) => {
		...
		function EvidenceDetails({ capability, snapshot, t }) {
			...
			const fields = [
				...
				[t("profilePersistence"), capability.configuration.profilePersistence.status === "writable" ? capability.configuration.profilePersistence.status : "reason" in capability.configuration.profilePersistence && capability.configuration.profilePersistence.reason !== void 0 ? `${capability.configuration.profilePersistence.status}: ${capability.configuration.profilePersistence.reason}` : capability.configuration.profilePersistence.status],
		...
		*/
		function capabilityHasAnomaly(capability, snapshot) {
			return capability.verification === "drifted" || capability.configuration.profileOverride.state === "unavailable" || capability.configuration.profilePersistence.status === "unwritable" || capability.runtimeState.lifecycle === "failed" || snapshot.compatibility.findings.some((finding) => finding.id === capability.id);
		}
		...
		function MutationControls({ capability, snapshot, busy, onMutate, t }) {
			...
			if (actions.length === 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				style: muted$1,
				children: snapshot.access.mutation === "loopback-required" ? t("remoteReadOnly") : `${t("controlsUnavailable")}: ${capability.mutationEligibility.reasons.map((reason) => t(`eligibility${capitalize$2(reason)}`)).join(", ") || t("unknown")}`
			});
		...
		*/
		...
		const zh = {
			...
			remoteReadOnly: "远程 Inspector 为只读；配置写入仅允许 loopback same-origin 请求。",
			...
			profileNotApplicable: "不适用（Agent Preset 组合）",
			...
			eligibilityAgentPresetScope: "Agent Preset 组合条目不是 Host mutation 目标",
			...
			lockExternal: "外部插件",
			lockAgentPreset: "Agent Preset 组合条目"
		};
		const en = {
			tab: "Built-ins",
