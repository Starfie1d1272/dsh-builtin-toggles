window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		//#endregion
		//#region src/client/catalog.en.ts
		...
		const enCatalog = Object.fromEntries(Object.keys(zhCatalog).map((id) => [id, {
			...ENGLISH_OVERRIDES[id],
			title: ENGLISH_OVERRIDES[id]?.title ?? titleFor(id),
			summary: ENGLISH_OVERRIDES[id]?.summary ?? summaryFor(id),
			category: "系统基础"
		}]));
