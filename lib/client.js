window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const PRESET_STATUS = "网页端顶层停用是正常状态；实际是否可用由当前会话的 Agent 预设决定。";
		...
		const zhCatalog = {
			...
			"tool-bash": presetManaged("Bash 工具", "让智能体在 macOS/Linux 等环境执行 Bash 命令。", "工具与执行"),
			"tool-pwsh": presetManaged("PowerShell 工具", "让智能体在 Windows 环境执行 PowerShell 命令。", "工具与执行"),
			...
			"skill-filesystem": presetManaged("本地 skill 发现", "从文件系统发现并注册当前智能体可用的 skill。", "模型与智能体"),
			"tool-skill": presetManaged("skill 工具", "让智能体查看和加载 skill，并处理用户显式调用的 skill。", "模型与智能体"),
			"tool-goal": presetManaged("目标工具", "让智能体读取和更新持久化的会话目标。", "模型与智能体"),
			"plan-mode": presetManaged("plan mode", "为智能体提供 plan mode、退出工具和对应的规划规则。", "模型与智能体"),
			...
			"tool-subagent": presetManaged("创建子代理", "让智能体创建一个可继续交互的 Spawn 子代理。", "模型与智能体"),
			"tool-subagent-fork": presetManaged("分叉（Fork）子代理", "让智能体从当前历史分叉一个一次性子代理。", "模型与智能体"),
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					visible.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						style: text,
						children: filters.anomaliesOnly ? hasActiveFilters ? t("searchEmptyAnomaliesFiltered") : t("searchEmptyAnomalies") : t("searchEmpty")
		...
		const zh = {
			...
			inspectorIntro: "检查当前 Host Loader 中的所有能力。组成检查是检查结论，写入资格是服务端对单项写入的独立授权；不可操作的条目仍保留在此处供检查。",
			searchPlaceholder: "搜索 ID、包名、类别或管理平面",
			searchEmpty: "没有匹配的能力。",
			searchEmptyAnomalies: "当前未发现异常项。",
			searchEmptyAnomaliesFiltered: "当前筛选条件下未发现异常项。",
			...
			verificationNotApplicable: "Agent 预设，不参与基线验证",
			...
			profileNotApplicable: "不适用（Agent 预设组合）",
			profileWritable: "可写",
			...
			eligibilityAgentPresetScope: "Agent 预设组合条目不是宿主写入目标",
		...
		const en = {
			...
			searchEmptyAnomaliesFiltered: "No anomalies found under the current filters.",
